...
public class FieldAndLocal {
    ...
    int a; // 필드(멤버 변수) 객체 생성시 자동으로 초기화됨

    void printNumber(int c) { // 매개변수(지역변수취급)
        int b = 10; // 지역변수 자동초기화 안됨
        System.out.println("필드 a = " + a);
        System.out.println("지역변수 b = " + b); //초기화하지 않은 지역변수는 사용할 수 없다.
        System.out.println("매개변수 c = " + c);
    }
    
    void foo(){
        System.out.println("a = " + a); // 필드는 어느 메서드에서든지 참조가 가능.
